...
class QuestionListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        # 用户点击tags栏时 需要选择一个tags
        context["active"] = "all"
        return context


# 已被回答的问题和未被回答的问题都在所有问题页中，所以直接继承QuestionListView
...
class CreateAnswerView(LoginRequiredMixin, CreateView):
    """回答问题"""
    model = Answer
    # 由于 字段只有一个 无标题 tags staus等 所以只使用fields
    fields = ['content', ]
    message = '您的问题已提交'
    ...
    def form_valid(self, form):
        # 提交评论的用户 = 登录用户
        form.instance.user = self.request.user
        # 评论的问题id = 前端传递的参数中 question_id 的值
...
# 删除问题
class QuestionDeleteView(LoginRequiredMixin,AuthorRequiredMixin, UpdateView):
    model = Question
