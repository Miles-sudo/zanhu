...
class MessagesListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        ).order_by('-last_login')[:10]  # -last_login 来自User 模型类继承的父类
        # 最近一次私信互动的用户
...
class ConversationListView(MessagesListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        # 用户在前端与指定用户聊天时 样式active 就变成指定用户 前端url 传递的参数
        context['active'] = self.kwargs["username"]
        return context

    # sql优化：只返回选中的用户
    def get_queryset(self):
        # 同理get_user_model() 返回的是 User模型类
...
def send_message(request):
    ...
    if len(message.strip()) != 0 and sender != recipient:
        ...
        # 视图中要传递给consumer的数据  让接收方看到的私信内容
        payload = {
            # type必须有的键值对 |  receive 代表的是 consumer.py中的receive函数
            'type': 'receive',
